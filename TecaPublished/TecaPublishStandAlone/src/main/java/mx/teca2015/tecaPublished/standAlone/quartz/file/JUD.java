...
import mx.teca2015.tecaPublished.standAlone.quartz.file.schede.UD;
 ...
 *
 */
public class JUD extends JFile<UD, MagXsd> {

	/**
	 * Variabile utilizzata per loggare l'applicazione
	 */
	private static Logger log = Logger.getLogger(JUD.class);

	public static String TYPE = "ud";
	...
	protected String jobExecute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			uc = new UD(finestra.getRow(1), f, folderIndex, genMagComp, genMagPub);
	...
	protected void publisher(String objectIdentifier, String filename, UD md, MagXsd mdXsd, IndexDocumentTeca admd)
		...
		try {
			...
			params.add(ItemTeca.TIPOLOGIAFILE, ItemTeca.TIPOLOGIAFILE_UD);
			params.add(ItemTeca.ORIGINALFILENAME, filename);

			if (md.getChildren() != null) {
				children = md.getChildren();
				keys = children.keys();
				while (keys.hasMoreElements()) {
					key = keys.nextElement();
					read(ItemTeca.CHILDREN, key);
					read(ItemTeca.CHILDRENDESC, children.get(key));
				}
			}
			...
			read(ItemTeca.TIPOLOGIAUNITAARCHIVISTICA, md.getTipologiaUnitaArchivistica());
			read(ItemTeca.TITOLO, md.getTitolo());
			read(ItemTeca.LINGUA, md.getLingua());
			read(ItemTeca.ANNOINIZIALE, md.getAnnoIniziale());
			read(ItemTeca.ANNOFINALE, md.getAnnoFinale());
			read(ItemTeca.SECOLOINIZIALE, md.getSecoloIniziale());
			read(ItemTeca.SECOLOFINALE, md.getSecoloFinale());
			read(ItemTeca.SUPPORTO, md.getSupporto());
			read(ItemTeca.CONSISTENZACARTE, md.getConsistenzaCarte());
			read(ItemTeca.STATOCONSERVAZIONE, md.getStatoConservazione());
			read(ItemTeca.DOCUMENTICARTOGRAFICI, md.getDocumentiCartografici());
